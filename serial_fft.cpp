...
// Based on iterative serial code retrieved from:
...
//So much less parameterization with this class based desine. Much easier to interpret and read.
class FFT {
  public:
...
int main(int argc, char *argv[]) {
   ...
   cout << "FFT perfomed on " << fft.duration << " second sample of " << fft.freq
             ...
             << "." << GetElapsed(sTime,fTime).tv_nsec << " seconds. "
             <<  endl;
   fft.fft_plot();
   return 0;
}  // end main
...
timespec GetElapsed(timespec sTime, timespec fTime) {
   ...
   if ((fTime.tv_nsec - sTime.tv_nsec) < 0) {
     ...
     // total.tv_nsec = 10000000 + fTime.tv_nsec - sTime.tv_nsec;
